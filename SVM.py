...
print(classification_report(TrainingLabel_test,prediction))
accuracy_score(TrainingLabel_test,prediction)
print("The f1-score for using only the Bag of Words features is : {}".format(classification_report(TrainingLabel_test,prediction)))
...
prediction_F1_TFIDF =model.predict(tfidf_Test)

from sklearn.metrics import classification_report, confusion_matrix
from sklearn.metrics import accuracy_score
print(classification_report(TrainingLabel_test,prediction_F1_TFIDF))
accuracy_score(TrainingLabel_test,prediction_F1_TFIDF)
print("The f1-score for using only the TFIDF features is : {}".format(classification_report(TrainingLabel_test,prediction_F1_TFIDF)))
...
prediction_generic = nb.predict(XTest_generic)
...
print("The accuracy score for using only the logistic regression model  features is : {}".format(Score))


from sklearn.metrics import classification_report, confusion_matrix
from sklearn.metrics import accuracy_score

print(classification_report(TrainingLabel_test,prediction_generic))
accuracy_score(TrainingLabel_test,prediction_generic)
print("The f1-score for not using TFIDF and Bag of words features is : {}".format(classification_report(TrainingLabel_test,prediction_generic)))
...
plt.title( 'Hate/NonHate distribution for the test data set using the labels predicted by the classifier model Bag of Words SVM')
plt.figure(10)
plt.pie(arr5, labels = mylabels5 ,autopct='%1.1f%%')
plt.title( 'Hate/NonHate distribution for the test data set using the labels predicted by the classifier model TFIDF SVM')
plt.figure(11)
plt.pie(arr6, labels = mylabels6,autopct='%1.1f%%')
plt.title('Hate/NonHate distribution for the test data set using the labels predicted by the classifier model  SVM ')

#plot model confidence scores using SVM
